...
II_SRC="$CONFIG_ROOT/illogical-impulse"

QS_DST="$HOME/.config/quickshell/ii"
HYPR_DST="$HOME/.config/hypr/hyprland"
II_DST="$HOME/.config/illogical-impulse"

BACKUP_ROOT="$HOME/punchcard/"
...
apply() {
  ...
  [[ -d "$II_SRC" ]] && echo "   Illogical Impulse (config.json)"
  [[ -d "$HYPR_SRC" ]] && echo "   Hyprland (general.conf, rules.conf)"
  ...
  backup_and_patch "$II_SRC" "$II_DST" "illogical-impulse"
...
restore() {
  ...
  restore_dir "illogical-impulse" "$II_DST"
...
- You can find the backups in ~/punchcard
